...
import 'dart:async';
import 'package:custom_dropdown_search/src/properties/clear_button_props.dart';
import 'package:custom_dropdown_search/src/properties/dropdown_button_props.dart';
...
typedef CustomCard<T> = Widget Function(T city, void Function(T itemToRemove));
...
class DropdownSearch<T> extends StatefulWidget {
  ...
  final Widget Function(Function onConfirm)? actionsBarBuilder;
...
class DropdownSearchState<T> extends State<DropdownSearch<T>> {
  ...
  Widget _defaultSelectedItemWidget() {
    Widget defaultItemMultiSelectionMode(T item) {
      return widget.itemCard != null
          ? widget.itemCard!(item, ((itemToRemove) {
              removeItem(item);
            }))
          : Container(
              height: 32,
        ...
        return ValueListenableBuilder<bool>(
            ...
            builder: (context, isFocused, w) {
              return InputDecorator(
                ...
                isFocused: isFocused,
                decoration: _manageDropdownDecoration(state),
                child: widget.popupProps.showSelectedItems == true
  ...
  Widget _popupWidgetInstance() {
    return SelectionWidget<T>(
      ...
      actionsBarBuilder: widget.actionsBarBuilder,
    );
  }
